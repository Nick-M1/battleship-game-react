...
export default function ProfileLarge({ profile, hasWon }: Props) {
    return (
        <div className='flex flex-col items-center'>
            ...
            <img src='/src/assets/battleships-icon.png' alt='profile-pic' title={profile.username} className={`mx-2 h-20 w-20 aspect-square rounded-full border-2 smooth-transition ${ hasWon ? 'border-teal-500' : 'border-gray-600' } `}/>
        </div>
    )
}